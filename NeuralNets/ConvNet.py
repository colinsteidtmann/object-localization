...
class ConvNet:
    def __init__(self, conv_method, layer_names, num_filters, kernel_sizes, stride_sizes, pool_sizes, pool_fns, pad_fns, activations, input_channels, scale_method=None, optimizer="adam", lr=0.01, lr_decay=0):
        ...
        if not (conv_method == "convolution" or conv_method == "cross_correlation"):
            sys.exit(
                'Ooops! ``conv_method`` must equal "convolution" or "cross_correlation" ')
        ...
        if (False in [True if (((type(kernelSize) is list and len(kernelSize) is 2) and (layer_name is "conv")) or ((kernelSize is None) and (layer_name is "pool"))) else False for kernelSize, layer_name in zip(kernel_sizes, layer_names)]):
            sys.exit(
                'Ooops! ``kernel_sizes`` can only contain list[kernel_height, kernel_width] in the same indexes as "conv" in ``layer_names``, None if layer_name index is "pool" ')
        if (False in [True if (((type(strideSize) is list and len(strideSize) is 2))) else False for strideSize in stride_sizes]):
            sys.exit(
                'Ooops! ``stride_sizes`` can only contain list of [stride_height, stride_width] for each layer_name in layer_names')
        if (False in [True if (((type(poolSize) is list and len(poolSize) is 2) and (layer_name is "pool")) or ((poolSize is None) and (layer_name is "conv"))) else False for poolSize, layer_name in zip(pool_sizes, layer_names)]):
            sys.exit(
                'Ooops! ``pool_sizes`` can only contain lists of [pool_height, pool_width] in the same indexes as "pool" in ``layer_names``, None if layer_name index is "conv" ')
        ...
        if (False in [True if (padFn == "same" or padFn == "valid") else False for padFn in pad_fns]):
            sys.exit(
                'Ooops! ``pad_fns`` can only contain lists of pad_fns (strings either "same", or "valid")')
        ...
        if (type(input_channels) is not int):
            sys.exit(
                'Ooops! ``input_channels`` must be an int (for rgb image, use 3)')
        ...
        self.initialize_weights(kernel_channels=[input_channels] + [(num_filters[self.get_last_convLayer_idx(
            idx)] if num_filters[idx] is not None else None) for idx in range(1, len(num_filters))], kernel_sizes=kernel_sizes)
        self.shared_funcs = SharedFunctions.SharedFunctions(activations, [k.shape for k in self.kernel_layers if k is not None], [
                                                            b.shape for b in self.bias_layers if b is not None], scale_method, optimizer, lr, lr_decay)
    ...
    def initialize_weights(self, kernel_channels, kernel_sizes):
        ...
        for layer_idx in range(len(self.layer_names)):
            if (self.layer_names[layer_idx] == "conv"):
                kernel_rows, kernel_cols, kernel_channel = kernel_sizes[layer_idx][
                    0], kernel_sizes[layer_idx][1], kernel_channels[layer_idx]
                layer_filters_num = self.num_filters[layer_idx]
                self.kernel_layers.append(np.random.randn(layer_filters_num, kernel_rows, kernel_cols,
                                                          kernel_channel)*np.sqrt(2/(layer_filters_num*kernel_rows*kernel_cols*kernel_channel)))
                self.bias_layers.append(
                    np.random.rand(layer_filters_num, 1, 1, 1))
            else:
    ...
    def feedforward(self, inputs, scale=True, test=False):
        ...
        if (type(inputs) is not np.ndarray or (len(inputs.shape) is not 4)):
            sys.exit(
                'Ooops! feedforward inputs must be 4d numpy array (batch_size, width, height, channels)')

        a = inputs
        if (self.scale_method is not None and scale == True and self.iteration_count > 0):
            inputs = self.shared_funcs.scale_fn(inputs, train=False)
    ...
    def apply_gradients(self, gradients):
        ...
        for nablaWs, nablaBs in gradients:
            new_kernels, new_biases = self.shared_funcs.optimize(nablaWs, nablaBs, [kernel_layer for kernel_layer in self.kernel_layers if kernel_layer is not None], [
                                                                 bias_layer for bias_layer in self.bias_layers if bias_layer is not None], self.iteration_count)
            self.kernel_layers, self.bias_layers = [new_kernels.pop(0) if kernel_layer is not None else None for kernel_layer in self.kernel_layers], [
                new_biases.pop(0) if bias_layer is not None else None for bias_layer in self.bias_layers]
    ...
    def gradients(self, inputs, dx_layer, dx_type, grad_ys=None, return_new_grad_ys=False):
        ...
        if type(dx_type) != list:
            dx_type = [dx_type]

        """ Check paramaters """
        if (type(inputs) is not np.ndarray or (len(inputs.shape) is not 4)):
            sys.exit(
                'Ooops! gradients inputs must be 4d numpy array (batch_size, width, height, channels)')
        ...
        """
        grad_ys = np.ones(np.array(z_layers[-1]).shape) if (
            grad_ys is None) else np.array(grad_ys).reshape(np.array(z_layers[-1]).shape)
        grad_ys *= self.shared_funcs.activation_fn_prime(
            z_layers[-1], self.get_last_convLayer_idx(len(self.layer_names)))
        ...
        for layer_idx, layer_name in reversed(list(enumerate(self.layer_names))):
            ...
            nabla_dwb_layer_idx = np.squeeze(
                np.argwhere(dx_layer == layer_idx+2))
            """ nabla dx layer idx for hidden layers and inputs """
            nabla_dh_layer_idx = np.squeeze(
                np.argwhere(dx_layer == layer_idx+1))

            last_conv_layer_idx = self.get_last_convLayer_idx(layer_idx)
            if layer_name == "conv":
                idx = len(delta_layers)
                if last_conv_layer_idx != -1:
                    delta = self.get_delta_derivs(self.shared_funcs.activation_fn_prime(
                        z_layers[-idx - 1], last_conv_layer_idx), delta_layers[-1], layer_idx)
                    if (nabla_dwb_layer_idx.size > 0 and "weights" in dx_type):
                        """Append kernel deriv to the correct nabla_dwb_layer_idx location """
                        nabla_dx_layer[dx_type.index("weights")][nabla_dwb_layer_idx] = (self.get_kernel_derivs(self.shared_funcs.activation_fn(
                            z_layers[-idx - 1], last_conv_layer_idx), delta_layers[-1], layer_idx) + self.shared_funcs.weight_decay_deriv(self.kernel_layers[layer_idx]))
                else:
                    delta = self.get_delta_derivs(
                        z_layers[-idx - 1], delta_layers[-1], layer_idx)
                    if (nabla_dwb_layer_idx.size > 0 and "weights" in dx_type):
                        """Append kernel deriv to the correct nabla_dwb_layer_idx location """
                        nabla_dx_layer[dx_type.index("weights")][nabla_dwb_layer_idx] = (self.get_kernel_derivs(
                            z_layers[-idx - 1], delta_layers[-1], layer_idx) + self.shared_funcs.weight_decay_deriv(self.kernel_layers[layer_idx]))

                if (nabla_dwb_layer_idx.size > 0 and "biases" in dx_type):
                    """Append bias deriv to the correct nabla_dwb_layer_idx location """
                    if (self.layer_names[-1] == "conv"):
                        nabla_dx_layer[dx_type.index("biases")][nabla_dwb_layer_idx] = np.sum(
                            delta_layers[-1], axis=(1, 2)).reshape((-1,) + self.bias_layers[layer_idx].shape)
                    else:
                        """ Go back 3 to grad y if last layer is pool """
                        nabla_dx_layer[dx_type.index("biases")][nabla_dwb_layer_idx] = np.sum(
                            delta_layers[-3], axis=(1, 2)).reshape((-1,) + self.bias_layers[layer_idx].shape)

                delta_layers.append(delta)

                idx = len(delta_layers)
                delta = self.get_pad_derivs(
                    z_layers[-idx - 1], delta_layers[-1], layer_idx)
                delta_layers.append(delta)
                if (nabla_dh_layer_idx.size > 0 and ("input" in dx_type or "zLayer" in dx_type)):
                    ...
                    nabla_dx_layer[dx_type.index(
                        "zLayer")][nabla_dh_layer_idx] = delta
            elif layer_name == "pool":
                idx = len(delta_layers)
                delta = self.get_pool_derivs(
                    z_layers[-idx - 1], z_layers[-idx], delta_layers[-1], layer_idx)
                if last_conv_layer_idx != -1:
                    delta *= self.shared_funcs.activation_fn_prime(
                        z_layers[-idx - 1], last_conv_layer_idx)
                delta_layers.append(delta)
        ...
        nabla_dx_layer = [list(filter(lambda a: a is not 0, x))
                          for x in nabla_dx_layer]
    ...
    def convolution(self, input_layer, layer_idx):
        ...
        num_inputs, num_rows, num_cols = input_layer.shape[
            0], input_layer.shape[1], input_layer.shape[2]
        num_filters, kernel_rows, kernel_cols, num_channels = kernel_layer.shape[
            0], kernel_layer.shape[1], kernel_layer.shape[2], kernel_layer.shape[3]
        stride_rows, stride_cols = stride_size[0], stride_size[1]
        output_rows, output_cols = int(
            ((num_rows - kernel_rows) / stride_rows) + 1), int(((num_cols - kernel_cols) / stride_cols) + 1)
        output_layer = np.zeros(
            (num_inputs, output_rows, output_cols, num_filters))
        for output_row in range(output_rows):
            for output_col in range(output_cols):
                z = np.array(np.tensordot(input_layer[:, stride_rows * output_row:stride_rows * output_row + kernel_rows, stride_cols *
                                                      output_col:stride_cols * output_col + kernel_cols, :], np.rot90(kernel_layer, 2, axes=(1, 2)), axes=((1, 2, 3), (1, 2, 3))), ndmin=2)
    ...
    def pool(self, input_layer, layer_idx):
        ...
        num_inputs, num_rows, num_cols, num_channels = input_layer.shape[
            0], input_layer.shape[1], input_layer.shape[2], input_layer.shape[3]
        pool_rows, pool_cols, stride_rows, stride_cols = pool_size[
            0], pool_size[1], stride_size[0], stride_size[1]
        output_pool_rows = int(((num_rows - pool_rows) / stride_rows) + 1)
        output_pool_cols = int(((num_cols - pool_cols) / stride_cols) + 1)
        output_layer = np.zeros(
            (num_inputs, output_pool_rows, output_pool_cols, num_channels))
        for output_pool_row_idx in range(output_pool_rows):
            for output_pool_col_idx in range(output_pool_cols):
                input_start_row, input_start_col = (
                    stride_rows * output_pool_row_idx), (stride_cols * output_pool_col_idx)
                if (pool_fn == "max"):
                    z = np.squeeze(np.max(input_layer[:, input_start_row:(input_start_row + pool_rows), input_start_col:(
                        input_start_col + pool_cols), :], axis=(1, 2))).reshape((num_inputs, num_channels))
                elif (pool_fn == "mean"):
                    z = np.squeeze(np.mean(input_layer[:, input_start_row:(input_start_row + pool_rows), input_start_col:(
                        input_start_col + pool_cols), :], axis=(1, 2))).reshape((num_inputs, num_channels))
                output_layer[:, output_pool_row_idx,
                             output_pool_col_idx, :] = z
        return output_layer
    ...
    def pad(self, input_layer, layer_idx):
        ...
        """ Set local variables """
        if (self.layer_names[layer_idx] == "conv"):
            filter_size = [self.kernel_layers[layer_idx].shape[1],
                           self.kernel_layers[layer_idx].shape[2]]
        ...
        num_inputs, input_rows, input_cols, num_channels = input_layer.shape[
            0], input_layer.shape[1], input_layer.shape[2], input_layer.shape[3]
        filter_rows, filter_cols, stride_rows, stride_cols = filter_size[
            0], filter_size[1], stride_size[0], stride_size[1]
        if (padding_fn == "same"):
            width_padding = (input_cols * stride_cols) + \
                filter_cols - input_cols - stride_cols
            height_padding = (input_rows * stride_rows) + \
                filter_rows - input_rows - stride_rows
            ...
            padded_inputs = np.pad(input_layer, [(0, 0), (col_padding_top, col_padding_bottom), (
                row_padding_left, row_padding_right), (0, 0)], mode='constant')
        elif (padding_fn == "valid"):
            max_num_rows = (int)(
                ((input_rows - filter_rows) / stride_rows) + 1)
            max_num_cols = (int)(
                ((input_cols - filter_cols) / stride_cols) + 1)
            padded_inputs = input_layer[:, :(filter_rows + (stride_rows * (
                max_num_rows - 1))), :(filter_cols + (stride_cols * (max_num_cols - 1))), :]
    ...
    def get_delta_derivs(self, input_layer, delta_layer, layer_idx):
        ...
        num_inputs, input_rows, input_cols, input_channels = input_layer.shape[
            0], input_layer.shape[1], input_layer.shape[2], input_layer.shape[3]
        num_filters, kernel_rows, kernel_cols = kernel_layer.shape[
            0], kernel_layer.shape[1], kernel_layer.shape[2]
        ...
        for delta_row in range(delta_rows):
            for delta_col in range(delta_cols):
                z = np.squeeze(np.tensordot(delta_layer[:, delta_row:delta_row + 1, delta_col:delta_col + 1, :], np.rot90(
                    kernel_layer, 2, axes=(1, 2)), axes=((3), (0)))).reshape((num_inputs, kernel_rows, kernel_cols, input_channels))
                delta_derivs[:, stride_rows * delta_row:stride_rows * delta_row + kernel_rows,
                             stride_cols * delta_col:stride_cols * delta_col + kernel_cols, :] += z
        return delta_derivs
    ...
    def get_kernel_derivs(self, input_layer, delta_layer, layer_idx):
        ...
        num_filters, kernel_rows, kernel_cols, kernel_channels = kernel_shape[
            0], kernel_shape[1], kernel_shape[2], kernel_shape[3]
        num_inputs, delta_rows, delta_cols = delta_layer.shape[
            0], delta_layer.shape[1], delta_layer.shape[2]
        stride_rows, stride_cols = stride_size[0], stride_size[1]
        kernel_derivs = np.zeros(
            (num_inputs, num_filters, kernel_rows, kernel_cols, kernel_channels))

        for delta_row in range(delta_rows):
            for delta_col in range(delta_cols):
                inputs = copy.deepcopy(input_layer)
                deltas = copy.deepcopy(delta_layer)
                inputs = np.repeat(np.array(inputs[:, stride_rows * delta_row:stride_rows * delta_row + kernel_rows,
                                                   stride_cols * delta_col:stride_cols * delta_col + kernel_cols, :], ndmin=5), num_filters, 0)
                deltas = np.repeat(np.array(
                    deltas[:, delta_row:delta_row + 1, delta_col:delta_col + 1, :], ndmin=5), kernel_channels, 0)
    ...
    def get_pool_derivs(self, input_layer, pool_layer, delta_layer, layer_idx):
        ...
        pool_rows, pool_cols, stride_rows, stride_cols = pool_size[
            0], pool_size[1], stride_size[0], stride_size[1]
        num_inputs, num_pool_rows, num_pool_cols, num_channels = pool_layer.shape[
            0], pool_layer.shape[1], pool_layer.shape[2], pool_layer.shape[3]
        delta_rows, delta_cols = delta_layer.shape[1], delta_layer.shape[2]
        ...
        for pool_row_idx in range(delta_rows):
            for pool_col_idx in range(delta_cols):
                ...
                input_start_row, input_start_col = (
                    stride_rows * pool_row_idx), (stride_cols * pool_col_idx)
                if (pool_fn == "max"):
                    i, r, c, ch = np.where(np.isin(input_layer[:, input_start_row:(input_start_row + pool_rows), input_start_col:(
                        input_start_col + pool_cols), :], pool_layer[:, pool_row_idx, pool_col_idx, :]))
                    input_layer_derivs[i, input_start_row + r, input_start_col +
                                       c, ch] += delta_layer[i, pool_row_idx, pool_col_idx, ch]
                elif (pool_fn == "mean"):
                    input_layer_derivs[:, input_start_row:input_start_row+pool_rows, input_start_col:input_start_col+pool_cols, :] += (
                        delta_layer[:, pool_row_idx, pool_col_idx, :]/(pool_rows*pool_cols)).reshape((num_inputs, 1, 1, num_channels))
    ...
    def get_pad_derivs(self, input_layer, delta_layer, layer_idx):
        ...
        if (padding_fn == "same" and input_layer.shape != delta_layer.shape):
            ...
            col_padding_bottom = int(np.ceil(height_padding / 2))
            col_padding_top = int(np.floor(height_padding / 2))
            padded_inputs_derivs = delta_layer[:, col_padding_top:(
                -col_padding_bottom if col_padding_bottom != 0 else None), row_padding_left:(-row_padding_right if row_padding_right != 0 else None), :]
        elif (padding_fn == "valid" or input_layer.shape == delta_layer.shape):
            max_num_rows = (int)(
                ((input_rows - filter_rows) / stride_rows) + 1)
            max_num_cols = (int)(
                ((input_cols - filter_cols) / stride_cols) + 1)
            cut_bottom_rows = input_rows - \
                (filter_rows + (stride_rows * (max_num_rows - 1)))
            cut_right_cols = input_cols - \
                (filter_cols + (stride_cols * (max_num_cols - 1)))
            padded_inputs_derivs = np.pad(delta_layer, [(
                0, 0), (0, cut_bottom_rows), (0, cut_right_cols), (0, 0)], mode='constant')
    ...
    def reset_params(self):
        ...
        self.shared_funcs = SharedFunctions.SharedFunctions(self.activations, [k.shape for k in self.kernel_layers if k is not None], [
                                                            b.shape for b in self.bias_layers if b is not None], self.scale_method, self.optimizer, self.lr, self.lr_decay)
