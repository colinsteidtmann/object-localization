...
class NeuralNetwork:
    ...
    def sgd_fit(self, inputs, labels=None, grad_ys=None, batch_size=100, epochs=1, train_pct=1.0, print_epoch=False, shuffle_inputs=True, random_states=None):
        ...
        if (train_pct < 0.0 or train_pct > 1.0):
            sys.exit('Ooops! train_pct must be a float between 0 and 1 (inclusive)')
        self.sgd_test_costs = []
        ...
        for epoch in range(epochs):
            for step in range(0, len(inputs), batch_size):
                ...
                train_x = inputs[step:step +
                                 math.floor(batch_size * train_pct)]
                train_y = None if labels is None else labels[step:step+math.floor(
                    batch_size * train_pct)]
                self.set_nprandomstates(random_states)
                train_predictions = self.feedforward(train_x, scale=True)

                if labels is not None:
                    test_x, test_y = inputs[step+math.floor(batch_size * train_pct):step+batch_size], labels[step+math.floor(
                        batch_size * train_pct):step+batch_size]
                    ...
                    if (train_pct != 1.0):
                        self.set_nprandomstates(random_states)
                        test_predictions = self.feedforward(
                            test_x, scale=True, test=True)
                        pct_correct = np.mean(np.squeeze(
                            np.argmax(test_predictions, 1)) == np.argmax(test_y, 1))
                        correct.append(pct_correct)
                        avg_correct = sum(correct[-10:]) / len(correct[-10:])
                        if avg_correct > 0.95:
                            break
                        print(test_predictions.shape, test_y.shape)
                        print(np.squeeze(np.argmax(test_predictions, 1)) == np.argmax(
                        ...
                        self.sgd_test_costs.append(
                            [(epoch + 1) * ((step+batch_size)/batch_size), np.sum(self.get_losses(test_predictions, test_y))])
                    else:
                        self.sgd_test_costs.append([np.nan, np.nan])
                    self.sgd_train_costs.append(
                        [(epoch + 1) * ((step+batch_size)/batch_size), np.sum(self.get_losses(train_predictions, train_y))])

                """ Perform SGD on training data """
                train_loss_grad = self.loss_prime(
                    train_predictions, train_y) if labels is not None else grad_ys
                grads = self.gradients(train_x, list(range(len(self.sections))), [
                                       "weights", "biases"], train_loss_grad, random_states)
                for section_idx, section in enumerate(self.sections):
                    ...
                    """
                    normalized_nablaWs = [
                        np.mean(nabla_wLayer, 0) for nabla_wLayer in nablaWs]
                    normalized_nablaBs = [
                        np.mean(nabla_bLayer, 0) for nabla_bLayer in nablaBs]
                    section.apply_gradients(
                        zip([normalized_nablaWs], [normalized_nablaBs]))
                ...
                if (self.regularizer == "early_stopping"):
                    if ((np.min(np.array(self.sgd_test_costs)[:, 1]) < np.min(np.array(self.sgd_test_costs)[-self.patience:, 1])) and ((epoch + 1) * ((step+batch_size)/batch_size)) > self.patience):
                        if print_epoch:
                            print("SGD stopped early at ", epoch)
                        break
    ...
    def gradients(self, inputs, section_layer, section_type, grad_ys=None, np_randomstates=None):
        ...
        """
        if not(type(section_layer) is list or type(section_layer) is np.ndarray):
            section_layer = [section_layer]
        if not (type(section_type) is list or type(section_type) is np.ndarray):
            section_type = [section_type]

        input_array = []
        a = copy.deepcopy(inputs)
        """ insures same output as outputs calculated for grad_ys"""
        if np_randomstates is not None:
            self.set_nprandomstates(np_randomstates)
        for section in self.sections:
            input_array.append(a)
            a = section.feedforward(a)

        returned_gradients = [[0 for section_type in section_type]
                              for section_layer in section_layer]
        """ insures same output as outputs calculated for grad_ys"""
        if np_randomstates is not None:
            self.set_nprandomstates(np_randomstates)
        for section_idx, (section, section_inputs) in reversed(list(enumerate(zip(self.sections, input_array)))):
            ...
            if type(section) is cn.ConvNet:
                if ("weights" in section_type or "biases" in section_type):
                    dx_layer.extend(
                        [idx + 2 for idx, layer_name in enumerate(section.layer_names) if layer_name is "conv"])
                if ("zLayer" in section_type):
                    dx_layer.extend(
                        [idx + 1 for idx, layer_name in enumerate(section.layer_names) if layer_name is "conv"])
            elif type(section) is fc.FullyConnected:
                if ("weights" in section_type or "biases" in section_type):
                    dx_layer.extend(
                        np.arange(2, (len(section.activations) + 1)))
                if ("zLayer" in section_type):
                    dx_layer.extend(
                        np.arange(0, (len(section.activations) + 2)))
    ...
    def check_gradients(self, inputs, labels=None, grad_ys=None):
        ...
        random_states = self.get_nprandomstates()
        if labels is None:
            calculated_gradients = self.gradients(
                inputs, [0, 1], gradient_types, grad_ys, random_states)
        else:
            self.set_nprandomstates(random_states)
        ...
        for section_idx, section in reversed(list(enumerate(self.sections))):
            ...
            for gradient_type_idx, gradient_type in enumerate(gradient_types):
                ...
                for idx in range(len(section_params)):
                    if section_params[idx] is None:
                        calculated_gradients[section_idx][gradient_type_idx].insert(
                            idx, None)
                ...
                for param_layer_idx, param_layer in enumerate(section_params):
                    if param_layer is not None:
                        ...
                        flattened_param_gradients = calculated_gradients[section_idx][gradient_type_idx][param_layer_idx].reshape(
                            (num_inputs, -1), order='F')
                        ...
                        for param_idx in range(len(flattened_param_layer)):
                            ...
                            plus_params = plus_params.reshape(
                                param_layer_shape, order='F')
                            ...
                            if labels is not None:
                                plus_output = self.loss(plus_output, labels)
                            ...
                            minus_params = minus_params.reshape(
                                param_layer_shape, order='F')
                            ...
                            if labels is not None:
                                minus_output = self.loss(minus_output, labels)
                            if grad_ys is not None:
                                minus_output *= grad_ys

                            """ Approx. gradient = (f(x+h)−f(x−h))/(2h) """
                            approximated_param_gradient = (
                                (plus_output - minus_output) / (2 * epsilon))
                            approximated_grads = approximated_param_gradient.reshape(
                                (num_inputs, -1)).sum(1)

                            calculated_grads = np.squeeze(
                                flattened_param_gradients[:, param_idx])
                            ...
                            if (np.allclose(np.squeeze(approximated_grads), np.squeeze(flattened_param_gradients[:, param_idx]), rtol=1e-7) is False):
                                print("Approximated gradients DO NOT equal Calculated gradients for {} in section:{}, layer:{}, param_idx:{}".format(
                                    gradient_type, section_idx, param_layer_idx, param_idx))
                                sys.exit("Approximated gradients:{}\nCalculated gradients:{}\nDifference:{}".format(
                                    approximated_grads, calculated_grads, (calculated_grads-approximated_grads)))
                            ...
                            print("Approximated gradients equal Calculated gradients for {} in section:{}, layer:{}, param_idx:{}".format(
                                gradient_type, section_idx, param_layer_idx, param_idx))
    ...
    def get_losses(self, predictions, labels):
        ...
        if not (isinstance(predictions, (list, np.ndarray)) and isinstance(labels, (list, np.ndarray))):
            sys.exit(
                'Ooops! inputs and labels for get_cost() must be passed as list or list in numpy array')
        costs = []
